...
namespace SharepointMigrations
{
    public class SharepointWrapper
    {
        ...
        public SharepointWrapper(string url, string user, string password)
        {
            this.url = url;
            var uri = new Uri(url);
            var cred = new SharePointOnlineCredentials(user, password.ToSecureString());

            clientContext = new ClientContext(uri) { Credentials = cred };
        }
        ...
        public IEnumerable<string> GetAllListItens(string listName)
        {
            try
            {
                ...
                return items.OfType<ListItem>().Select(f => f["Title"].ToString());
            }
            catch
            {
            }
        ...
        public void AddColumnUserMulti(string listName, string columnName)
        {
            ...
            CheckIfColumnNameIsAvailable(listName, columnName, list);
        ...
        public void AddColumnSingleLineText(string listName, string columnName)
        {
            //"<Field Type='Geolocation' DisplayName='Location'/>"
            var list = clientContext.Web.Lists.GetByTitle(listName);
            clientContext.Load(list);
        ...
        public void AddColumnMultiLineText(string listName, string columnName)
        {
            //"<Field Type='Geolocation' DisplayName='Location'/>"
            var list = clientContext.Web.Lists.GetByTitle(listName);
            clientContext.Load(list);
        ...
        private bool CheckIfColumnNameIsAvailable(string listName, string columnName, List list)
        {
            //TODO: check type
